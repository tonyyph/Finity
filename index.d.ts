...
type ButtonStyle = {
  ...
  textColor: ColorName
  disableBackgroundColor: ColorName
  disableTextColor: ColorName
  loadingBackgroundColor: ColorName
  isPrimary?: boolean
}
type BasicColor = 'black' | 'white' | 'transparent' | 'current' | 'inherit'
type ColorKey = Exclude<keyof import('tailwindcss/types/generated/colors').DefaultColors, BasicColor>
type ColorVariant = '50' | '100' | '200' | '300' | '400' | '500' | '600' | '700' | '800' | '900' | '950'
type ColorName = `${ColorKey}-${ColorVariant}` | BasicColor | TextCustomColor | BackgroundCustomColor | StatusCustomColor
type IconProps = {
  size?: number
  color?: ColorKey
  filled?: boolean
...
declare type NavigationProps<T extends keyof NavigationStackParamList> = ScreenProps<T>['navigation']

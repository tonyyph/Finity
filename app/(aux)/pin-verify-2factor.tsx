...
export default function PINVerify2FactorScreen() {
  ...
  return (
    <View style={tw`bg-white flex-1`}>
      ...
      <View style={tw`flex-1 px-sp16 pt-sp32`}>
        <View style={tw`z-10 mb-sp8 gap-sp8`}>
          <Typography type="hs" weight="semibold">
            {`Two-factor authentication`}
          </Typography>
          <Typography weight="regular">{`Enter the 6-digit verification code generated from\nyour app.`}</Typography>
        </View>
        <View style={tw`flex flex-row justify-between items-center mt-8 gap-sp8`}>
          {otp.map((digit, index) => (
      ...
      <View key={1} style={tw`justify-start px-sp16`}>

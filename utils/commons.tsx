...
const BASE_HEIGHT = 844 // iPhone 14 Pro size (your original base)
const BASE_WIDTH = 390 // iPhone 14 Pro size

/** Scale based on screen height */
export const scaleOnHeight = (value: number) => (SCREEN_HEIGHT / BASE_HEIGHT) * value

/** Scale based on screen width */
export const scaleOnWidth = (value: number) => (SCREEN_WIDTH / BASE_WIDTH) * value
...
export function memoFC<T extends ComponentType<any>>(component: T, ignoreProps?: string[]): MemoExoticComponent<T> {
 ...
 */
export const actionWithTimeout = <T extends any>(cb: () => Promise<T>, onCallback: (result: T | null) => void) => {
  return async () => {
    const timeout = new Promise<T>((_, reject) => setTimeout(() => reject(new Error('Operation timed out')), 5000))
...
export const exactDesign = (value: number) => {
  ...
  return Platform.OS === 'android' ? Math.round(PixelRatio.roundToNearestPixel(adjusted)) : Math.round(adjusted)
}
...
export function formatNumber({
  ...
  return `${negativeSign}${formatted}${decimalCount ? decimal + decimalPart : ''}`
}
